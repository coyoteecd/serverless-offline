...
class HttpServer {
  constructor(serverless, options, lambda) {
    ...
    if (typeof httpsProtocol === 'string' && httpsProtocol.length > 0) {
      serverOptions.tls = {
        cert: (0, _fs.readFileSync)((0, _path2.resolve)(httpsProtocol, 'cert.pem'), 'ascii'),
        key: (0, _fs.readFileSync)((0, _path2.resolve)(httpsProtocol, 'key.pem'), 'ascii')
      };
  ...
  createRoutes(functionKey, httpEvent, handler) {
    ...
    const endpoint = new _Endpoint.default((0, _path2.join)(_classPrivateFieldLooseBase(this, _serverless)[_serverless].config.servicePath, handlerPath), httpEvent);
    const {
      path
    } = httpEvent; // path must start with '/'

    let hapiPath = path.startsWith('/') ? path : `/${path}`;
    const _path = hapiPath; // prepend stage to path
    ...
    hapiPath = `/${stage}${hapiPath}`; // but must not end with '/'
      ...
      if (integration === 'AWS') {
        if (requestTemplate) {
          try {
            ...
            event = new _index.LambdaIntegrationEvent(request, _classPrivateFieldLooseBase(this, _serverless)[_serverless].service.provider.stage, requestTemplate, _path).create();
          } catch (err) {
        ...
        const lambdaProxyIntegrationEvent = new _index.LambdaProxyIntegrationEvent(request, _classPrivateFieldLooseBase(this, _serverless)[_serverless].service.provider.stage, _path);
  ...
  createResourceRoutes() {
    ...
    Object.entries(resourceRoutes).forEach(([methodId, resourceRoutesObj]) => {
      ...
      hapiPath = `/${_classPrivateFieldLooseBase(this, _options)[_options].stage || _classPrivateFieldLooseBase(this, _serverless)[_serverless].service.provider.stage}${hapiPath}`;
